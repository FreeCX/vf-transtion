...
class ffmpeg:
    def __init__(self, fps, size, output):
        self.cmd_out = [
            'ffmpeg', '-f', 'rawvideo', '-pix_fmt', 'rgb24',  '-video_size', f'{size[1]}x{size[0]}', '-r', str(fps),
            '-i', '-', '-c:v', 'libx264', '-preset', 'slow', '-profile:v', 'high', '-crf', '18', '-coder', '1',
            '-pix_fmt', 'yuv420p', '-vf', 'scale=iw:-2', '-movflags', '+faststart', '-g', '30', '-bf', '2', '-y',
            str(output)
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', dest='transition', metavar='seconds', type=int, default=3, help='transition time (default: 3)')
